...
\label{fig1}
\end{center}
\item le biais de prédiction
\end{itemize}
En effet, un modèle climatique est une simplification d'un système énormément plus complexe. De nombreux biais viennent donc entacher les résultats de ces modèles.\\
...
\right.$
\end{center}
Avec cette écriture, il est donc plus facile de comprendre d'où vient le biais (on retire $\frac{\lambda}{2}$ au coefficient des MCO) ainsi que la sélection de variables. On parle dans ce cas de \og \textbf{seuillage doux} \fg{}. La figure suivante permet d'illustrer le propos.
\begin{center}
