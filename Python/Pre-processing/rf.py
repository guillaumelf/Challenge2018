...
def tune_model(files,villes,train_drop,test_drop,replacement_month,replacement_date,regressor,params,dict_params,nb_cv_folds,output_file):
    for i in range(len(files)):
        ...
        filename = 'Results/RandomForest/'+villes[i]+'_results.csv'
...
liste_scores = []
f = open('Results/RandomForest/Best_tune.txt','w')
...
dict_params = {'n_estimators' : [800],'max_features': [7,9,11,13]}
params = {'bootstrap':True,'oob_score':False}
files = os.listdir('data_agg_sep')
villes = ['Toulouse','Bordeaux','Rennes','Lille','Nice','Strasbourg','Paris']

if __name__ == '__main__':              
    tune_model(files,villes,var_dropped_train,var_dropped_test,dico_transfo,dico_dates,ensemble.RandomForestRegressor,params,dict_params,5,f)   
